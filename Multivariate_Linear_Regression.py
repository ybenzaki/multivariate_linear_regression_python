...
fig = plt.figure(2)
ax = fig.add_subplot(1,2,1, projection='3d')
ax.scatter(df["taille_m_carre"], df["nb_chambres"], df["prix"], c='r', marker='^')
ax.plot_trisurf(df["taille_m_carre"], df["nb_chambres"], predict_all(df["taille_m_carre"], df["nb_chambres"]))
ax.set_xlabel('surface_m_carre')
ax.set_ylabel('nb_chambres')
ax.set_zlabel('prix en $')
plt.show()
